...
#reading in all the csv files. Objective here is to create some visualization of the energy usage and greenhouse gas emissions and obtain a summary
#of the columns.
...
#Dropping all the duplicate values based on Unique Attribute "Operation"

rawdata2014 <- subset(rawdata2014, !duplicated(subset(rawdata2014, select=c(Operation))))
rawdata2015 <- subset(rawdata2015, !duplicated(subset(rawdata2015, select=c(Operation))))
rawdata2016 <- subset(rawdata2016, !duplicated(subset(rawdata2016, select=c(Operation))))
rawdata2017 <- subset(rawdata2017, !duplicated(subset(rawdata2017, select=c(Operation))))
rawdata2018 <- subset(rawdata2018, !duplicated(subset(rawdata2018, select=c(Operation))))
rawdata2019 <- subset(rawdata2019, !duplicated(subset(rawdata2019, select=c(Operation))))

#Replacing all DIV/0 errors with NA's by coercion

rawdata2014$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2014$Energy.Intensity.GJ_m2))
rawdata2014$GHG.Emissions.KG <- as.numeric(as.character(rawdata2014$GHG.Emissions.KG))

rawdata2015$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2015$Energy.Intensity.GJ_m2))
rawdata2015$GHG.Emissions.KG <- as.numeric(as.character(rawdata2015$GHG.Emissions.KG))

rawdata2016$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2016$Energy.Intensity.GJ_m2))
rawdata2016$GHG.Emissions.KG <- as.numeric(as.character(rawdata2016$GHG.Emissions.KG))

rawdata2017$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2017$Energy.Intensity.GJ_m2))
rawdata2017$GHG.Emissions.KG <- as.numeric(as.character(rawdata2017$GHG.Emissions.KG))

rawdata2018$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2018$Energy.Intensity.GJ_m2))
rawdata2018$GHG.Emissions.KG <- as.numeric(as.character(rawdata2018$GHG.Emissions.KG))

rawdata2019$Energy.Intensity.GJ_m2 <- as.numeric(as.character(rawdata2019$Energy.Intensity.GJ_m2))
rawdata2019$GHG.Emissions.KG <- as.numeric(as.character(rawdata2019$GHG.Emissions.KG))

###2014

#For each year creating 4 variables for sum of each sector and using it to create a bar plot and pie chart
...
#Bar plot and pie chart for each. same process repeated for each year
...
#Summary of all the emissions and energy intensity columns by year
